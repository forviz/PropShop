...
export const contactAgent = (domain, name, emailFrom, emailTo, mobile, body, agentId, agentName, propertyId, projectName) => {
  return (dispatch) => {
    dispatch(contactAgentSuccess(domain, ''));
    dispatch(contactAgentSubmitting(domain, true));
    createContactAgent(name, emailFrom, emailTo, mobile, body, agentId, agentName, propertyId, projectName).then((result) => {
